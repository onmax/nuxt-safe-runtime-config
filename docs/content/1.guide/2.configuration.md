...
| ------------------- | ------------------------------- | ----------------- | --------------------------------- |
| `$schema`           | `StandardSchemaV1 \| string`     | -                 | Schema or default-export module path |
...
Inline runtime schemas use [@cfworker/json-schema](https://github.com/cfworker/cfworker/tree/main/packages/json-schema), a lightweight (~8KB) validator that works on all runtimes including edge. A string `$schema` path executes the original Standard Schema instead, so runtime transformations are preserved.
