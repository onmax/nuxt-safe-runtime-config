---
title: Zod
icon: simple-icons:zod
---

[Zod](https://zod.dev/) is a TypeScript-first schema validation library with a fluent API.

## Installation

```bash
npm install zod
```

## Basic Schema

```ts [nuxt.config.ts]
import { z } from 'zod'

export default defineNuxtConfig({
  modules: ['nuxt-safe-runtime-config'],
  runtimeConfig: {
    databaseUrl: '',
    secretKey: '',
    port: 3000,
    public: { apiBase: 'https://api.example.com', appName: 'My App' },
  },
  safeRuntimeConfig: {
    $schema: z.object({
      public: z.object({ apiBase: z.string(), appName: z.string().optional() }),
      databaseUrl: z.string(),
      secretKey: z.string(),
      port: z.number().optional(),
    }),
  },
})
```

## Common Patterns

### Required vs Optional

```ts
z.object({
  required: z.string(), // must exist
  optional: z.string().optional(), // can be undefined
  withDefault: z.number().default(3000), // default if undefined
  nullable: z.string().nullable(), // can be null
})
```

### String Validations

```ts
z.object({
  email: z.string().email(),
  apiUrl: z.string().url(),
  token: z.string().min(32),
  slug: z.string().regex(/^[a-z-]+$/),
  trimmed: z.string().trim(),
})
```

### Number Validations

```ts
z.object({
  port: z.number().int().min(1).max(65535),
  timeout: z.number().nonnegative(),
  ratio: z.number().positive(),
})
```

### Enums

```ts
z.object({
  logLevel: z.enum(['debug', 'info', 'warn', 'error']),
  cacheStrategy: z.enum(['memory', 'redis', 'none']),
})
```

### Nested Objects

```ts
z.object({
  public: z.object({
    api: z.object({
      baseUrl: z.string(),
      version: z.string(),
    }),
  }),
  redis: z.object({
    host: z.string(),
    port: z.number(),
  }),
})
```

### Transforms

Zod supports transforms. Use an importable schema module with `validateAtRuntime` to execute them against runtime overrides:

```ts
z.object({
  // Type will be inferred as string
  apiKey: z.string().transform(s => s.trim()),
})
```

Default-export that schema from a separate file and set `$schema` to its project-relative path.

## Full Example

```ts [nuxt.config.ts]
import { z } from 'zod'

export default defineNuxtConfig({
  modules: ['nuxt-safe-runtime-config'],
  safeRuntimeConfig: {
    $schema: z.object({
      public: z.object({
        apiBase: z.string().url(),
        appName: z.string(),
        logLevel: z.enum(['debug', 'info', 'warn', 'error']),
      }),
      database: z.object({ url: z.string(), poolSize: z.number().optional() }),
      auth: z.object({ jwtSecret: z.string().min(32), sessionTtl: z.number().positive() }),
      smtp: z.object({ host: z.string(), port: z.number().int().positive(), from: z.string().email() }).optional(),
    }),
  },
})
```
